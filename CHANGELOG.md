...
## [0.6.0] — 2026-09-10 (compose v2, recipe build, planning twins, plannable creates, v26)

**279 leaf commands across 42 domains** (was 267/39). Headline: a Meta campaign can now be described as a brief and built end-to-end (`recipe build`), every op a v2 plan can carry is now executable from a plan file (`plan apply --from-file` handles creates, not just updates), plan documents can be merged/forecast/exported/read back (`plan merge|forecast|export|get`), A/B tests run through a new `experiment` domain, a multi-account budget allocator ships (`agency portfolio plan`), and the Graph/Marketing API default moves to **v26.0**.

### Added
- **`apb context init|show`** — new domain (2 leaves). Local per-account planning state (`$APB_HOME/context/<act_id>.json`, schema v2: `goals`/`brand`/`lists`/`recipes`/`applied_actions[]`) — the same document shape `apb-gads context` writes, so a Meta and a Google context file are structurally interchangeable. No Graph call, no write gate.
- **`apb recipe list|describe|build`** — new domain (3 leaves). `recipe build --brief brief.yaml [--execute] [--plan out.html]` turns a YAML brief into a full compose-v2 spec, runs it through the same compiler `--print-compiled` uses, and writes `spec.v2.json` / `plan.json` / `plan.md` / `plan.html` / `preview.html` / `summary.json` (+ `launch.json` on `--execute`). `--execute` applies through the same plannable-creates executor a hand-built plan does — four write gates, temp-ref binding, receipt, rollback blueprint. Reference brief: `rust/tests/fixtures/brief-sandbox-meta.yaml`.
- **`CampaignComposeSpec v2`** (`campaign compose-from-spec`) — a **strict superset** of v1 (`schema_version` absent/`1` is unchanged). `targeting_brief` / `creative_brief` compile through the SAME builders the flag-driven `adset create` commands use, so a brief-authored ad set and a flag-authored one produce byte-identical `targeting`. `ad_sets[].advantage.audience` is required and explicit under v2 (mirrors v26's server-side requirement). Fail-loud on any brief/flag collision (`targeting`+`targeting_brief`, etc. — errors name both sides). `--print-compiled` runs the compiler locally with zero API calls for CI/review use.
- **Plannable creates** — `PlanAction` gained the create/delete class (`campaign.create`, `adset.create`, `creative.create`, `creative.delete`, joining the pre-existing `ad.create`/`*.delete`), so every op the Meta v2 vocabulary carries now has an executor variant and a rollback blueprint. `plan apply --from-file` binds `{{account}}`/`{{ref:aN}}` placeholders and executes atomically in stage order (campaign → ad sets → creatives → ads), reporting `status` (`EXECUTED`/`PARTIAL`/`FAILED`), `completed_through`, and a `rollback_envelope` per run. Write gate 5 now also fires on a v2 envelope's own `destructive_for()` classification, not just the file's own `destructive` flag — a tampered/hand-edited plan can no longer bypass it.
- **`adset update --advantage-audience on|off` / `--advantage-placements on`** — read-modify-write Advantage+ toggles over the live ad set (previous `advantage.*` flags existed only on `create`).
- **`apb plan merge --from a.json --from b.json … [--mode growth|efficiency] [--horizon 14d] [--learning-window-days N] --out merged.json [--plan merged.html]`** — merges N Meta plan-envelope-v2 files (v1 lifted transparently) through the shared `apb_envelope::merge` core. Budget/target/bidding op classes are read from Meta's real executor table; LEARNING-class ad sets (incl. `LEARNING_LIMITED` and any future `LEARNING_*` status) gate a merge until cleared or `--learning-window-days` elapses (default 7d, Meta's ~50-event Smart-Bidding heuristic).
- **`wait-for-status` honoured at apply time** — `plan apply --from-file` / `plan validate --from-file` re-read live ad-set learning status for every `wait-for-status` pseudo-action (never trust the merge-time snapshot); a gated action is imported as `status: "waiting-on-learning"` / `"skipped_waiting"` rather than executed early or silently dropped. `plan validate --from-file` is the new read-only twin of `plan apply --from-file` (same gates, zero mutation).
- **`apb plan forecast --adset-spec spec.v2.json[#ad_sets[N]] | --adset-id <id> [--budget-scenarios 50,75,100] [--period 30d] [--out forecast.json] [--plan forecast.html]`** — delivery-estimate budget scenarios (`estimate_mau_lower_bound`/`_upper_bound`/`_ready` only — `bid_estimate` is live-verified deprecated pre-v3.0 and deliberately never requested). An Advantage+/fully-automated ad set (no explicit geo/audience/interest targeting) is marked `unforecastable` with a stated reason rather than guessed. Feeds `score.{growth_micros,confidence,reason}` on the synthetic plan action. `budget simulate --adset-id … --daily-budget … [--basis auto|heuristic]` re-bases the pre-existing closed-form simulator onto a live `delivery_estimate` scenario when the ad set is forecastable, falling back to the heuristic (both report their `basis`).
- **`apb plan export --from-file plan.json --format json|md|html [--fonts web]`** — export/re-render any stored or file-based plan through the shared renderer without needing a plan id.
- **`apb plan get --id [--format record|v2]`** — CLI twin of `GET /api/v1/plans/:id`.
- **`apb experiment create --base-campaign-id … --traffic-split 50 [--treatment-plan plan.json] [--hypothesis …] [--duration-days N] [--execute]` / `experiment results --id [--metric conversions|cost_per_conversion|conversion_value_per_cost] [--confidence 0.95]` / `experiment promote --id --plan p.json` / `experiment end --id --plan p.json`** — new domain wrapping the existing split-test service. `experiment results` runs a two-proportion z-test on conversions-per-click (gated at `p<0.05` AND a 14-day runtime floor) for a `WINNER`/`LOSER`/`INCONCLUSIVE` verdict with a `days_needed` extrapolation when inconclusive. `plan merge` now rewrites a `full` learning-reset-class change (`adset.update-targeting`/`-optimization-goal`/`-bid`/`-bidding`) into `experiment.create` when the target ad set's 30-day conversion volume clears the readiness floor (10 conversions), rather than applying a disruptive change directly against a delivering ad set.
- **`apb agency portfolio plan [--accounts act_a,act_b] [--objective …] [--max-step-pct N] [--budget-total N --currency USD]`** — local marginal-return budget allocator across ad sets: 30d/60d ROAS-slope scoring, LEARNING/low-spend ad sets excluded with reasons, `bid_constraints.roas_average_floor` gates increases donor-only, a live `budget simulate --basis auto` headroom probe fails open, per-currency group-median split (never cross-currency), one `adset.update-budget` plan action per move. `--meta-portfolio-out` writes a bridge file for `apb-gads portfolio plan --with-meta` to consume.
- **`--fonts system|web`** — global flag threaded into every HTML plan render site (`--plan out.html`, `plan forecast --plan`, `plan export --format html`, `recipe build`'s `plan.html`) for a consistent web-font choice across every plan document.

### Changed
- **Meta Graph / Marketing API default pinned to `v26.0`** (was `v25.0`), after reading the [v26.0 changelog](https://developers.facebook.com/docs/graph-api/changelog/version26.0) end-to-end on 2026-09-09. Breaking items absorbed: `delivery_estimate` no longer requests the removed `daily_outcomes_curve` / `budget_guardrail` / `estimate_dau` (nor `bid_estimate`); `instagram_positions: "explore"` and `messenger_positions: "story"` are dropped with a `placement_removed_in_v26` advisory on **every** ad-set create and update path (an emptied positions array is removed, not sent as `[]`); `targeting_automation.advantage_audience` is now emitted explicitly on `adset update-targeting` as well as every create path (carried forward from the live ad set when the spec omits it) so v26's `ADS_TARGETING__REQUIRE_EXPLICIT_ADVANTAGE_AUDIENCE_FLAG` can't fire. **Rollback:** set `META_GRAPH_VERSION=v25.0` — no rebuild; v25.0 is served until 2028-07-29. Full diff: `rust/docs/META_API_FIELDS.md` § "v25.0 → v26.0 diff".
- **Compose v2 creative briefs** gain a `shop_ads_default_v26` advisory when no `destination_spec` is set (v26 defaults an eligible creative to `WEBSITE_AND_SHOP` for advertisers with a shop; `WEBSITE_AND_SHOP_OPT_OUT` keeps website-only), and a `destination_type_unknown` advisory for values outside the documented set.
- **`budget simulate --basis auto|heuristic`** — see `plan forecast` above; every simulation now reports which basis produced its number.

### Fixed
- **`pixel list` / `pixel get` / `pixel health` no longer fail on accounts that can't serve `match_rate_approx`.** Meta answers the whole request with HTTP 500 `code: 1` on those accounts; the field is now requested optimistically and, on a field-class failure, retried without it — rows come back with `match_rate_approx: null` and `match_rate_note: "unavailable on this account"`. `pixel get` degrades once further, to identity + status fields with a `fields_note`, on accounts that also 500 on the pixel statistics fields.
- **Ad-Budget-Optimization (ABO) compose now sends an explicit `is_adset_budget_sharing_enabled`** on a campaign create with no campaign-level budget — previously 400'd on action 1 of every ABO compose ("You must specify True or False…").
- **`recipe build` places the bid strategy where the budget is** (campaign under CBO, ad set under ABO) — Meta refuses a campaign-level bid strategy on a budget-less campaign.
- **`learning_status_for_adset` now matches every `LEARNING_*` status**, not just the literal string `"LEARNING"` — `LEARNING_LIMITED` (ad set below the ~50-event/7d threshold, a common real-world status) previously fell through to `Unknown` and let `plan merge`/wait-for-status proceed against a still-learning ad set with no wait.

### Notes
- **Plannable creates ship without live dispatch for `experiment.promote`/`experiment.end`.** `plan apply --from-file --execute` recognizes `experiment.*` ops (imported as `status: "experiment_action"`, never a hard error) but does not yet dispatch them to the split-test service — never exercised live (no delivering sandbox account). A future release should either wire the dispatch or make `plan apply` refuse those ops explicitly until it does.
- **`apb-gads plan forecast`** ships alongside this release on the Google line (`gads-v0.3.0`+) — see `CHANGELOG-gads.md`.
...
### Added
- **`scripts/apb/` on the public repo** — the plan-first-cli-001 batch-script library (Tier 1 watchdogs, Tier 2 opportunity scans, Tier 3 reviews, generated `audit-*` bundles, the shared `lib/` + `thresholds.conf`, and `README.md`) now ships to end users. `release.yml` rsyncs the whole `public-scripts/` tree (`--delete`-scoped to `scripts/apb/` so removals propagate); nothing outside that subtree is published (never-publish list intact).
- **`public/data/scripts-catalogue.json`** — generated by the new `rust/scripts/gen_scripts_catalogue.py` (reads BOTH the apb and gads `public-scripts/` trees), one entry per script + library file with `{name, binary, tier, cadence, description, writes, file, sha256, download_url}`. `--check`-gated in `rust-quality.yml` so a manifest-header edit, a new/removed script, or a changed checksum can't drift from the catalogue.
- **Docs** — CLAUDE.md § Repository topology + AGENTS.md § Release process now record that the release workflows publish `scripts/{apb,gads}/` in addition to `bin/` and the CHANGELOGs; the apb Claude skill gains a "Batch scripts" section.
