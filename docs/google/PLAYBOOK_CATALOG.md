...
| `competitor-pressure` | IMPLEMENTED | 30d | Trend the impression-share family (rank-lost + top + absolute-top) over N windows and decompose loss into rank-lost (→ raise bid / improve QS / tighten match) vs budget-lost (→ raise budget), emitting a `under_pressure`/`relief`/`steady` verdict. Knobs: `--windows` (2), `--level campaign\|ad_group\|keyword` (campaign), `--under-pressure-delta` (0.05), `--under-pressure-floor` (0.10). IS clamping handled (`<10%`/`>90%` never read as literals). **Honest limit: named-competitor / overlap / outranking data is NOT available via the Google Ads API at any access tier — this is the impression-share proxy; use the Google Ads UI Auction Insights report for competitor domains.** |
...
| `search-term-analysis` | IMPLEMENTED | 90d | Consolidated search-term analysis: ties each search term to its triggering keyword (text + match type), computes per-term CPA/ROAS, and emits BOTH promote candidates (high-ROI search terms not yet keywords → keyword adds) and negate candidates (high-cost/zero-conversion terms → negatives) in one combined `--output-spec` envelope for the dry-run-first apply pipeline (`plan from-audit` → `changes from-plan` → `changes apply`). |
| `search-term-ngram-audit` | IMPLEMENTED | 90d | Decomposes search terms into 1/2/3-grams, aggregates spend/clicks/conversions per n-gram, and flags wasteful patterns (high spend, ~0 conversions) + the most efficient ones. Emits the wasteful **multi-word** n-grams as `shared_negative_candidates` to a shared negative list via the actionable pipeline (`--shared-set` required); 1-grams are surfaced but only emitted as negatives with `--include-unigram-negatives`. |
...
| `campaign-type-fit` | IMPLEMENTED | 30d | Per-ENABLED-campaign channel-type fit vs conversion signal: flags Demand Gen optimizing for conversions with no signal (→ Search) and PMax launched onto thin signal (→ Search-first). The type-level companion to the verdict; aligns with verdict G3. Read-only. |
...
| `bid-strategy-mismatch` | IMPLEMENTED | 30d | Detect campaigns whose *current* bid strategy is throttling scale. Fires 10 named rules with per-rule severity + evidence block: `manual_cpc_with_strong_conversions`, `max_clicks_with_strong_conversions`, `tcpa_budget_throttled`, `tcpa_target_too_low`, `troas_without_value_tracking` (CRITICAL when value tracking unhealthy), `maximize_conversions_without_target_cpa`, `learning_stuck`, plus the S5 smart-bidding-misapplication trio — `tcpa_insufficient_conversions` (TARGET_CPA below `--tcpa-min-conversions`, default 30 → recommend Maximize Conversions), `troas_insufficient_conversions` (TARGET_ROAS below `--troas-min-conversions`, default 15 → recommend Maximize Conversion Value, or Maximize Conversions if value tracking is unhealthy), `max_conv_value_insufficient_conversions` (MAXIMIZE_CONVERSION_VALUE below `--max-conv-value-min-conversions`, default 15 → keep uncapped). The trio frames the fallback as the correct entry strategy for the volume tier (best-practice guidance, not an API gate), cites the lookback window, and names `mutate campaign-update-bidding-strategy` for the manual switch — advisory only. Two GAQL probes (campaign metrics + conversion-action value-health). |
...
| `pmax-url-exclusion-audit` | IMPLEMENTED | 90d | Substitute for v24-removed `url_expansion_opt_out`: per (PMAX/DISPLAY campaign × common-waste URL pattern) coverage check against `campaign_criterion` WEBPAGE negatives (v25-correct — `customer_negative_criterion.webpage.*` doesn't exist). Patterns `/blog,/careers,/privacy,/terms,/about,/affiliate,/coupon,/jobs,/legal` by default. Mutation-ready `customer_negative_criterion_webpage_candidates` spec via `--output-spec`. |
