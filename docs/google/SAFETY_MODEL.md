...
proposing the write. See `CAPABILITY.md` for which surfaces are proven at which tier.
...
So `apb-gads validate campaign-spec --from-file s.json && apb-gads orchestrate campaign-launch --from-file s.json` halts before launch on a bad spec. More in `CLI_AUTOMATION.md`.
