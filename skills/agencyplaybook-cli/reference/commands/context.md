...
Create or update the account's context file. Repeatable list flags REPLACE what is on disk; omitting a flag PRESERVES it, so a goal-only re-init never silently wipes the account's brand rules

| Flag | Value | Description |
|---|---|---|
| `--mode` | `<MODE>` | Goal mode: `target_cpa` \| `target_roas` \| `maximize_conversions` \| `maximize_conversion_value` \| `lowest_cost`. Preserved when omitted |
| `--target-cpa` | `<TARGET_CPA>` | Target cost per action in account currency |
| `--target-roas` | `<TARGET_ROAS>` | Target return on ad spend (e.g. `2.5`) |
| `--primary-kpi` | `<PRIMARY_KPI>` | Primary KPI recorded in the context (default `conversions`) |
| `--brand-term` | `<TERM>` | Brand term (repeatable). Recipes that need brand rules stop rather than guessing a brand from campaign names |
| `--brand-domain` | `<DOMAIN>` | Brand domain (repeatable) |
| `--brand-competitor` | `<BRAND>` | Competitor brand (repeatable) |
| `--json` |  | Output as JSON |
...
```bash
apb context init --mode <MODE> --target-cpa <TARGET_CPA>
```

### `apb context show`

Print the account's context file
...
```bash
apb context show --plan <PATH> --fonts <MODE>
```
