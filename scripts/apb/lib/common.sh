# common.sh — shared runtime for the apb (Meta) Tier-1 watchdog scripts.
#
# Sourced, never executed. It provides: arg parsing, binary discovery, the apb
# invocation wrapper (adds --json --no-input --no-color and fails loud on a
# non-zero binary exit), dated status-file I/O under ./apb-watch/<account>/<date>/,
# a yesterday-diff helper, and the 0 (quiet) / 10 (attention items) / 1 (tool
# error) exit-code convention shared by every watchdog.
#
# writes: never — every apb call this file makes is a read; it appends nothing to
# any account and never passes the write/execute rail.
#
# Portability: bash 4+, jq, and the apb binary on PATH (or APB_BIN=/path/to/apb).
...
# --account is required (or env APB_WATCH_ACCOUNT); --lookback defaults per script
# (a watchdog sets LOOKBACK before calling parse_common_args).
: "${ACCOUNT:=${APB_WATCH_ACCOUNT:-}}"
: "${LOOKBACK:=30}"
: "${OUT_DIR:=}"
: "${QUIET:=0}"
: "${WATCH_NAME:=watchdog}"
...
common_usage() {
  cat >&2 <<EOF
${WATCH_NAME} — read-only Meta (apb) watchdog. Flags findings "for weekly
review"; it never proposes or applies a change.

Usage: ${WATCH_NAME}.sh --account <act_id> [--lookback <days>] [--out-dir <dir>] [--quiet]

  --account   Ad account (act_XXXX). Defaults to \$APB_WATCH_ACCOUNT.
  --lookback  Lookback window in days (script default ${LOOKBACK}).
  --out-dir   Status-file directory (default ./apb-watch/<account>/<YYYY-MM-DD>/).
  --quiet     Suppress progress on stderr.

Env: APB_BIN=/path/to/apb overrides binary discovery.
...
parse_common_args() {
  ...
  command -v "$APB_BIN" >/dev/null 2>&1 || { echo "ERROR: apb binary not found ($APB_BIN); set APB_BIN=/path/to/apb" >&2; exit 1; }
  [ -n "$ACCOUNT" ] || { echo "ERROR: --account <act_id> required (or env APB_WATCH_ACCOUNT)" >&2; exit 1; }

  [ -n "$OUT_DIR" ] || OUT_DIR="./apb-watch/${ACCOUNT}/$(date +%F)"
  mkdir -p "$OUT_DIR"
  _log "watchdog=${WATCH_NAME} account=${ACCOUNT} lookback=${LOOKBACK}d out=${OUT_DIR}"
}

_log() { [ "$QUIET" = "1" ] || echo "$*" >&2; }

# run_apb <apb args...> — injects --account/--json/--no-input/--no-color, echoes
# progress to stderr, returns stdout, and exits 1 (tool error) on a non-zero exit.
run_apb() {
  _log "  -> apb $*"
  local out rc
  set +e
  out="$("$APB_BIN" "$@" --account "$ACCOUNT" --json --no-input --no-color 2>/dev/null)"
  rc=$?
  set -e
  if [ "$rc" -ne 0 ]; then
    echo "ERROR: apb $* exited ${rc} — tool/auth error" >&2
    exit 1
  fi
  printf '%s' "$out"
}

# status_write <name> <json> — persist today's status file.
status_write() {
  local name="$1" json="$2"
  mkdir -p "$OUT_DIR"
...
# status_yesterday <name> — echo yesterday's status file (default layout) if present.
status_yesterday() {
  local name="$1" yday f
  yday="$(date -d 'yesterday' +%F 2>/dev/null || date -v-1d +%F 2>/dev/null || true)"
  [ -n "$yday" ] || return 0
  f="./apb-watch/${ACCOUNT}/${yday}/${name}.json"
  [ -f "$f" ] && cat "$f" || return 0
}

# attn <msg> — emit an [ATTN] line (stdout) and count it for the exit code.
attn() {
  printf '[ATTN] %s\n' "$*"
  _ATTN_COUNT=$((_ATTN_COUNT + 1))
}

# finish — exit 10 if any attention item was flagged, else 0.
finish() {
...
# helpers below build on. apb keys everything off the ad account; the gads
# common.sh defines the same two functions off --customer.
review_subject() { printf '%s' "$ACCOUNT"; }
watch_root()     { printf './apb-watch/%s' "$ACCOUNT"; }
...
# audit_record <slug> <json> — persist one playbook's captured JSON (apb is
# --json-to-stdout, so the generated line captures stdout and hands it here) and
# tally ok/fail. Re-checks selection so an unselected slug is a silent skip.
audit_record() {
  local slug="$1" json="${2:-}"
...
Usage: $1.sh --account <act_id> [--lookback <days>] [--only <slug,slug>] [--out-dir <dir>] [--quiet]
