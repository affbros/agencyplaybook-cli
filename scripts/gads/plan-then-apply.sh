...
# parity linter scans $APB_GADS_BIN / run_gads invocations, and there are
# deliberately none here to bind the apply flag to. The apply flag itself is
# never spelled in this source (see learn_apply_flag); it is learned from the
# binary and re-typed by the operator at runtime, invisible to any static scan.
BIN="${APB_GADS_BIN:-apb-gads}"
APPLY_SUB1="mutate"
APPLY_SUB2="apply-plan"
: "${GADS_EXTRA_ARGS:=}"
...
design). It never composes the apply flag itself — you type it. BYO credentials
via GADS_EXTRA_ARGS="--config /path/to/google-ads.yaml".
EOF
}
...
command -v "$BIN" >/dev/null 2>&1 || { echo "ERROR: apb-gads binary not found ($BIN); set APB_GADS_BIN=/path/to/apb-gads" >&2; exit 1; }
...
learn_apply_flag() {
  ...
  "$BIN" ${GADS_EXTRA_ARGS} "$APPLY_SUB1" "$APPLY_SUB2" --help 2>&1 | grep -oE -- '--exec[a-z]+' | head -n1
}
...
echo "  (add --config/--customer as your setup requires; requires your write env gates enabled)"
echo
...
# shellcheck disable=SC2086  # GADS_EXTRA_ARGS is intentionally word-split.
"$BIN" ${GADS_EXTRA_ARGS} "$APPLY_SUB1" "$APPLY_SUB2" --from-file "$twin" "$typed_flag"
